...
def denoise_frame(frame):
    ...
    gaussian_kernel = np.ones((3, 3), np.float32) / 9   # We used 3x3 kernel
    denoised_frame = cv2.filter2D(frame, -1, gaussian_kernel)   # Applying filter on frame
